...
def init_sqlalchemy(dbname='sqlite:///media_information.db'):
    global engine
...
def test_sqlalchemy_add_all(n=100000):
    init_sqlalchemy()
    t0 = time.time()
    DBSession.add_all([Customer(name="NAME " + str(i)) for i in range(n)])
    DBSession.commit()
    print(
        "SQLAlchemy ORM add_all(): Total time for " + str(n) +
        " records " + str(time.time() - t0) + " secs")


if __name__ == '__main__':
    # 从上到下，由慢到快
    test_sqlalchemy_add_all(100000)
